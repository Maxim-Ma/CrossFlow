...
model = Args(
    ...
    # full_ckpt = '../exp1125_dFM_SSclip_arc5-11m-vaeV1.1B11pt500K-6-2-KLv5mp3v1.1en2_imgGT5.2_lr2en5_cfgLeInd/workdir/t2i_featureSSclip_dfm_small_arcH5-11m/default/ckpts/70000.ckpt/nnet_ema.pth',
    stage_configs = [
            Args(
...
def get_config():
    ...
    config.optimizer = d(
        name='adamw',
        lr=0.00005, # 0.0002 - clip; 0.0001 - general training; 0.00002 - finetuning
        weight_decay=0.03,
        betas=(0.9, 0.9),
    )
    ...
    global model
    config.nnet = d(
        name='mr_ju_t2i_textTransVAE_timm',
        model_args=model,
    )
    config.loss_coeffs = [1/4, 1]
    
    config.dataset = d(
        name='JDB_full_features',
        resolution=256,
        llm='clip',
        path='../../dataset/JDB_train/',
        val_path='../../dataset/SS13M_256_full_features/',
        cfg=False,
        p_uncond=0.1
    )
    ...
    config.sample = d(
        sample_steps=50,
        n_samples=30000,
        mini_batch_size=40,  # the decoder is large
